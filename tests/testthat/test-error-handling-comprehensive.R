...
test_that("Option validation errors", {
  ...
  expect_error(opts_read_json(digits_promote = -1))
  expect_error(opts_read_json(digits_promote = 31))
  ...
  expect_error(opts_write_json(digits_secs = -1))
  expect_error(opts_write_json(digits_secs = 7))
...
test_that("Edge case data handling", {
  ...
  # Special numeric values in different contexts
  expect_equal(read_json_str('"NA"', num_specials = "special"), NA_real_)
  expect_equal(read_json_str('"NA"', num_specials = "string"), "NA")
  expect_equal(read_json_str('"Inf"', num_specials = "special"), Inf)
  expect_equal(read_json_str('"Inf"', num_specials = "string"), "Inf")
  expect_equal(read_json_str('"-Inf"', num_specials = "special"), -Inf)
  expect_equal(read_json_str('"NaN"', num_specials = "special"), NaN)
...
test_that("Advanced flag combinations", {
  ...
  # Big number as raw flag
  big_num <- "99999999999999999999999999999999999999"
  expect_equal(
    read_json_str(big_num, yyjson_read_flag = yyjson_read_flag$YYJSON_READ_BIGNUM_AS_RAW),
    big_num
  )
})
